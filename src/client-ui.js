...
window.__ModuleLoader__.load({id:'@hanamesh/dsh-app-host',factory:function(require){'use strict';
  ...
  let libraryVisible=false,coreService=null;const libraryListeners=new Set();
  ...
  function LibraryOverlay(){
    const[visible,setVisible]=React.useState(libraryVisible),[snapshot,setSnapshot]=React.useState(null),[error,setError]=React.useState(''),[receipt,setReceipt]=React.useState(null),[account,setAccount]=React.useState('未安装 HanaMesh Core');
    React.useEffect(()=>{libraryListeners.add(setVisible);return()=>libraryListeners.delete(setVisible);},[]);
    const load=React.useCallback(async()=>{try{setError('');setSnapshot(await request('/hanamesh/library'));}catch(e){setError(String(e.message??e));}},[]);
    React.useEffect(()=>{if(visible){void load();if(coreService?.getSession)Promise.resolve(coreService.getSession()).then(session=>setAccount(session?'Core 已登录':'Core 未登录'),()=>setAccount('Core 状态不可用'));}},[visible,load]);
    const operate=async(path,body)=>{try{setError('');await request(path,{method:'POST',body:JSON.stringify(body)});await load();}catch(e){setError(String(e.message??e));}};
    ...
    const items=snapshot?.items??[],installed=snapshot?.installed??[];
    return h('section',{className:'hm-library-overlay'},h('header',null,h('div',null,h('h1',null,'应用库'),h('small',null,account)),h('button',{type:'button',onClick:()=>setLibraryVisible(false)},'关闭')),
      ...
      h('div',{className:'hm-library-grid'},...items.map(item=>{const row=item.installed;let action=h('span',null,item.application?'可安装':'只可浏览');if(item.application&&!row)action=h('button',{type:'button',onClick:()=>void operate('/hanamesh/library/install',{itemId:item.id})},'安装');else if(row?.state==='runtime-missing')action=h('button',{type:'button',onClick:()=>void operate('/hanamesh/library/provision',{appId:row.appId,packageName:row.packageName,runtimeItem:row.runtimeItem})},'补齐运行时');else if(row?.state==='registered')action=h('button',{type:'button',onClick:()=>void open(row)},'打开');else if(row)action=h('span',null,'已安装，重启 DSH 后可打开');return h('article',{key:item.id},h('h3',null,item.displayName),h('p',null,item.summary),h('small',null,(item.publisher?.name??'未知发布者')+' · '+(item.latestVersion??'—')),action);})),
  ...
  function apply(ctx){coreService=ctx.get?.('hanameshCore')??null;ctx.effect(()=>{const style=document.createElement('style');style.textContent='.hm-providers,.hm-library-sources{display:grid;gap:16px}.hm-providers table{width:100%;border-collapse:collapse}.hm-providers th,.hm-providers td{padding:8px;text-align:left;border-bottom:1px solid color-mix(in srgb,currentColor 14%,transparent)}.hm-provider-card{padding:16px;border:1px solid color-mix(in srgb,currentColor 18%,transparent);border-radius:12px}.hm-provider-row{display:grid;grid-template-columns:minmax(120px,1fr) auto minmax(180px,1fr) auto;gap:8px;align-items:center;margin:8px 0}.hm-source-row{display:grid;grid-template-columns:minmax(0,1fr) repeat(4,auto);gap:8px}.hm-source-row code{overflow-wrap:anywhere}.hm-source-add{display:flex;gap:8px}.hm-source-add input{flex:1}.hm-library-overlay{position:absolute;inset:0;z-index:30;pointer-events:auto;overflow:auto;padding:24px;background:#fff;color:#18181b}.hm-library-overlay button,.hm-library-overlay input{color:#18181b}.hm-library-overlay>header{display:flex;justify-content:space-between;align-items:center}.hm-library-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:16px}.hm-library-grid article{display:grid;gap:10px;padding:16px;border:1px solid color-mix(in srgb,currentColor 16%,transparent);border-radius:12px}.hm-app-frame{display:grid;grid-template-rows:auto 1fr;height:calc(100vh - 130px)}.hm-app-frame>div{display:flex;justify-content:space-between}.hm-app-frame iframe{width:100%;height:100%;border:0}';document.head.append(style);return()=>style.remove();},'hanamesh-app-host:style');
