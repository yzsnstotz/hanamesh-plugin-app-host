...
window.__ModuleLoader__.load({id:'@hanamesh/dsh-app-host',factory:function(require){'use strict';
  ...
  function LibraryAction(){return h('button',{type:'button',className:'hm-library-action',onClick:()=>setLibraryVisible(true)},'市场');}
  ...
  function RestartNotice({reason}){
    ...
    return h('p',{role:'status',className:'hm-restart-notice','data-hanamesh-restart':'required'},h('strong',null,'需重启 DSH'),' ',reason??'插件更改在 DSH 重启后生效。',' ',
      shell?h('a',{href:RESTART_LINK,className:'hm-restart-link',onClick:event=>{event.preventDefault();globalThis.window.location.assign(RESTART_LINK);}},'重启 DSH'):h('span',{className:'hm-restart-hint'},'请手动重启 DSH。'));
  }
  function LibraryOverlay(){
    const[visible,setVisible]=React.useState(libraryVisible),[snapshot,setSnapshot]=React.useState(null),[error,setError]=React.useState(''),[receipt,setReceipt]=React.useState(null),[query,setQuery]=React.useState(''),[filter,setFilter]=React.useState(''),[extra,setExtra]=React.useState([]),[pending,setPending]=React.useState({}),[seenCategories,setSeenCategories]=React.useState([]),[changed,setChanged]=React.useState(false);
    ...
    return h('section',{className:'hm-library-overlay hm-market'},h('header',null,h('div',null,h('h1',null,'HanaMesh 市场')),h('button',{type:'button',onClick:()=>setLibraryVisible(false)},'关闭')),
      ...
      h('div',{className:'hm-library-grid'},...items.map(card)),
      nextCursor&&!receipt?h('button',{type:'button',className:'hm-library-more',onClick:()=>void more()},'更多'):null,
      !receipt&&(installed.length||plugins.length)?h('section',{className:'hm-market-installed'},h('h2',null,'已安装'),h('ul',null,
