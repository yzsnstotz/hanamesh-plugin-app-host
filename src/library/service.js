...
export function createLibraryService({domain,host,config={},dataRoot,ledgerReader,installer}){
  ...
  // Real sources hold ~10k entries (2026-09-20: market.hanamesh.com 12,121). The library is an *application* library, so it
  // browses `category=hanamesh-app` by default; `q`/`cursor` pass through to the provider page; `category=''` lists everything.
