...
>
> 应用库浏览可只配置 `library.fixture` 或 `library.sources`；`sources` 未配置时默认 HanaMesh 目录源（rc.26）。安装/卸载需要绝对 `library.profileDir`、`library.profileName`、`library.dshBin` 与 `nodeBinary`——桌面壳 overlay 显式给出；纯 DSH CLI profile 下 rc.26 从本包自身真实安装位置、启动本进程的 `@deepseek-ai/dsh/lib/bin.js` 和（非 Electron 的）`process.execPath` 推断，显式配置优先。钉版本宿主没有公开 profile-directory 服务，因此仍不从 cwd、PATH 或个人 `~/.dsh` 猜。目录源与安装契约见 `LIBRARY.md`，推断规则见 `CONTRACT.md`「应用库配置」。
