...
const cases=[
  ...
  {id:'M11-usage-denied-counts',file:'src/gateway.js',from:"if (response.statusCode !== 401 && response.statusCode !== 403) this.#forwarded();",to:"this.#forwarded(); /* MUTATION: an app's own 401/403 answer counts as usage */",test:'tests/usage-evidence.test.mjs',pattern:'^AH-U02:',reason:/Expected values to be strictly equal|strictly deep-equal/},
  {id:'M12-usage-hour-bucket',file:'src/router/receipts.js',from:"if (!includeCurrent && item.hour >= current) continue;",to:"if (false) continue; /* MUTATION: the running hour is reported before its count is final (every request could become a use event) */",test:'tests/usage-evidence.test.mjs',pattern:'^AH-U02:',reason:/the running hour is not reported yet|Expected values to be strictly equal|strictly deep-equal/},
  {id:'M15-receipt-count-not-accumulated',file:'src/router/receipts.js',from:"item[field] += 1; item.firstAt = Math.min(item.firstAt, at);",to:"item[field] = 1; item.firstAt = Math.min(item.firstAt, at); /* MUTATION: a receipt never counts past one request */",test:'tests/usage-evidence.test.mjs',pattern:'^AH-U08',reason:/Expected values to be strictly equal|strictly deep-equal/},
  {id:'M16-router-inject-observer-blocks-launch',file:'src/router/broker.js',from:"if(routes.length){try{onInject({appId,instanceId,routes});}catch{}}",to:"if(routes.length){onInject({appId,instanceId,routes});} /* MUTATION: a failing receipt observer fails the credential injection */",test:'tests/router-broker.test.mjs',pattern:'^AH-R12',reason:/observer down|Expected values to be strictly equal/},
  {id:'M13-market-suite-protected',file:'src/library/install.js',from:"const blocked=new Set(PROTECTED_PACKAGES);",to:"const blocked=new Set(); /* MUTATION: the market may remove hanamesh-core / app-host itself */",test:'tests/library-market.test.mjs',pattern:'^AH-M02:',reason:/Missing expected rejection|PACKAGE_DENIED/},
  {id:'M14-provision-input-400',file:'src/library/service.js',from:"requireCondition(text(input?.runtimeItem,160),'INVALID_INPUT','runtimeItem is required.');",to:"requireCondition(true,'INVALID_INPUT','runtimeItem is required.'); /* MUTATION: a missing runtimeItem starts the operation and fails inside it (LIB-PROVISION-INPUT) */",test:'tests/library-market.test.mjs',pattern:'^AH-M06',reason:/Missing expected rejection|INVALID_INPUT/},
  {id:'M17-market-seat-exclusive',file:'src/client-ui.js',from:"const conflict=marketSeatTaken(ctx)||await dshmarketInstalled();",to:"const conflict=false; /* MUTATION: the market seat is taken even when dshmarket already holds or owns it */",test:'tests/market-seat.test.mjs',pattern:'^AH-MS0[23]',reason:/the seat is never contested|a registered seat is never overwritten|Expected values to be strictly deep-equal/},
